...
transit_data_chain* initRowChain(mem_data_head* colChain){
    string sequence=colChain->sequence;
...
bool colTOrow(mem_data_head** dataToTransit,transit_data_chain** rowChain){
    ...
    //外循环，循环row链表结点  
        ...
        //内循环，循环dataToTransit链表
            ...
            //每次内循环完成之后，释放已经转换了的node的内存，然后将下一个节点指定为firstNode;
            saveHead=colChainHeadPointer;
            colChainHeadPointer->firstNode=colChainHeadPointer->firstNode->next;
            colChainHeadPointer=colChainHeadPointer->next;
            if(colChainHeadPointer!=NULL)
                colChainNodePointer=colChainHeadPointer->firstNode;
            if(i==sequence.size()){
                delete(saveHead);
            }
