...
bool makeIndex(string filePath,string sequence){
    ...
    int nameLength=filePath.size()-17;
    string indexPath=filePath.substr(0,filePath.size()-(nameLength+3));
    fileName.resize(fileName.size()-3);
    fileName.erase(0,fileName.size()-nameLength);
    indexName.erase(0,indexName.size()-(nameLength+3));
    indexName.resize(nameLength);
    ...
    //写入尾时间戳
    fin.seekg(sizeof(time_t)*((pow(10,getFileLevel(filePath)))-1),ios::beg);
    fin.read((char*)&timeStamp,sizeof(time_t));
    fout.write((char*)&timeStamp,sizeof(time_t));
    fin.close();
    //开始根据索引写入数据偏移
    int sequenceNum;
    //略过10^level个时间戳数据
    int pos=sizeof(time_t)*(pow(10,getFileLevel(filePath)));
    for(int i=1;i<=sequence.size();i++){
        ...
        case 1:
            ...
            pos+=sizeof(int)*(pow(10,getFileLevel(filePath)));
            break;
        case 2:
            //是char类型数据
            fout.write((char*)&pos,sizeof(int));
            pos+=sizeof(char)*(pow(10,getFileLevel(filePath)));
            break;
        case 3:
            //是bool类型数据
            fout.write((char*)&pos,sizeof(int));
            pos+=sizeof(bool)*(pow(10,getFileLevel(filePath)));
            break;
        case 4:
            //是float类型数据
            fout.write((char*)&pos,sizeof(int));
            pos+=sizeof(float)*(pow(10,getFileLevel(filePath)));
            break;
        case 5:
            //是double类型数据
            fout.write((char*)&pos,sizeof(int));
            pos+=sizeof(double)*(pow(10,getFileLevel(filePath)));
            break;
        default:
            break;
        }
