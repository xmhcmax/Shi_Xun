...
bool is_fileFull(string filePath){
    //首先查看是不是最高级目录
    string path_save=filePath;
    if(stoi(path_save.erase(0,path_save.size()-1))==3){
        //说明是最高级目录，超过10个文件也不管
        return false;
    }
    //读取已经有了的该级文件
...
int getFileLevel(string filePath){
    ...
    if(filePath==".db"){
        int nameLength=fileLevel_s.size()-17;
        //说明是数据文件
        fileLevel_s.resize(fileLevel_s.size()-(nameLength+2));
...
string makeFileName(string filePath){
    ...
    if(is_fileFull(filePath)){
        //说明该级文件满10个了，要进行upgrade
    ...
    if(_findnext(Handle, &fileInfo)!= 0){
        ...
        // do
        // {
        //     string name_save=fileInfo.name;
        //     nameLength=name_save.size()-5;
        //     //查看是不是index文件
        //     if(name_save.erase(0,nameLength+2)==".db"){
        //         //是就保存这次文件名
        //         name=fileInfo.name;
        //         continue;
        //     }
        ...
        int fileNum=1;
        name=filePath.erase(0,filePath.size()-1);
        while(_findnext(Handle, &fileInfo) == 0){
            fileNum++;
        }
        nameNumber=to_string((fileNum/2));
        name.append(".");
        name.append(nameNumber);
        name.append(".db");
        return name;
...
bool upgradeFile(string filePath){
    ...
    //是最高级目录(不用判断是否满了，肯定可以继续放)，或者非最高级但未满目录(可以放下这次升级的的文件)，就不用递归了
    //这个时候就要根据sequence来读取了,读取上一级的文件号最低的文件的索引，找出sequence
    oldIndexPath.append("\\");
    oldIndexPath.append(to_string(level_i-1));
    ...
    //打开新文件
    ofstream fout;
    fout.open(newFilePath,ios::binary|ios::app);
    //设置初始偏移
    int pos=sizeof(string)*2+sizeof(time_t)*2-sizeof(int);//因为时间戳完了之后也是要加一次sizeof(int)的，所以事先减去
    int off;
    int size=pow(10,level_i-1);
    //定义好数据容器
    time_t timeData[size];
    int intData[size];
    char charData[size];
    bool boolData[size];
    float floatData[size];
    ...
    for(int i=0;i<=sequence.size();i++){
        ...
        for(int j=0;j<10;j++){
            ...
            oldFilePath.append("\\");
            oldFilePath.append(to_string(stoi(level_s)-1));
            oldFilePath.append(".");
            ...
            if(i==0){
                //转移时间戳数据
                fin_file.read((char*)timeData,sizeof(time_t)*(pow(10,(level_i-1))));
                fout.write((char*)timeData,sizeof(time_t)*(pow(10,(level_i-1))));
                fin_file.close();
                //pos+=sizeof(time_t)*(pow(10,(level_i-1)));
                continue;
            }
            else{
                //根据索引转移数据
                ...
                case 1:
                    ...
                    fin_file.read((char*)intData,sizeof(int)*(pow(10,(level_i-1))));
                    //写入数据
                    fout.write((char*)intData,sizeof(int)*(pow(10,(level_i-1))));
                    fin_file.close();
                    if(i==sequence.size()){
                ...
                case 2:
                    ...
                    //根据偏移找到数据
                    fin_file.seekg(off,ios::beg);
                    fin_file.read((char*)charData,sizeof(char)*(pow(10,(level_i-1))));
                    //写入数据
                    fout.write((char*)charData,sizeof(char)*(pow(10,(level_i-1))));
                    fin_file.close();
                    if(i==sequence.size()){
                        //说明是最后一个数据了，移动完了就要删除文件了
                        remove(oldFilePath.c_str());
                ...
                case 3:
                    ...
                    //根据偏移找到数据
                    fin_file.seekg(off,ios::beg);
                    fin_file.read((char*)boolData,sizeof(bool)*(pow(10,(level_i-1))));
                    //写入数据
                    fout.write((char*)boolData,sizeof(bool)*(pow(10,(level_i-1))));
                    fin_file.close();
                    if(i==sequence.size()){
                        //说明是最后一个数据了，移动完了就要删除文件了
                        remove(oldFilePath.c_str());
                ...
                case 4:
                    ...
                    //根据偏移找到数据
                    fin_file.seekg(off,ios::beg);
                    fin_file.read((char*)floatData,sizeof(float)*(pow(10,(level_i-1))));
                    //写入数据
                    fout.write((char*)floatData,sizeof(float)*(pow(10,(level_i-1))));
                    fin_file.close();
                    if(i==sequence.size()){
                        //说明是最后一个数据了，移动完了就要删除文件了
                        remove(oldFilePath.c_str());
                ...
                case 5:
                    ...
                    //根据偏移找到数据
                    fin_file.seekg(off,ios::beg);
                    fin_file.read((char*)doubleData,sizeof(double)*(pow(10,(level_i-1))));
                    //写入数据
                    fout.write((char*)doubleData,sizeof(double)*(pow(10,(level_i-1))));
                    fin_file.close();
                    if(i==sequence.size()){
                        //说明是最后一个数据了，移动完了就要删除文件了
                        remove(oldFilePath.c_str());
