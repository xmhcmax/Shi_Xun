...
void getADataToColNode(mem_data_node* node,string sequenceNum,mem_data_struct* off){
    ...
    case 0:
        //读出一个时间戳数据
        //time_t timeStamp;
        node->data->timeStamp=(time_t)&off;
        off+=sizeof(time_t);
        break;
    case 1:
        //读出一个int类型
    ...
    if(chainHead->last==NULL){
        ...
        for(int i=0;i<=sequence.size();i++){
            ...
            if(i=0){
                ...
                getADataToColNode(node,"0",off);
            }
            getADataToColNode(node,departSequence(sequence,i),off);
...
mem_data_head* addOneHead(mem_data_head* chainHead){
    addHead(chainHead);
}
